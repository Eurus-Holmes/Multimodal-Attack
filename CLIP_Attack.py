...
def retrieval_eval(model, model_wrapper, data_loader, tokenizer, device, config):
    # test
    model.float()
    model.eval()
    # ref_model.eval()
    ...
    text_attacker = BERTAttackLi2020.build(model_wrapper)
...
def main(args, config):
    ...
    ref_model = BertForMaskedLM.from_pretrained(args.text_encoder)
    model_wrapper = textattack.models.wrappers.HuggingFaceModelWrapper(ref_model, tokenizer)
    ...
    score_i2t, score_t2i = retrieval_eval(model, model_wrapper, test_loader, tokenizer, device, config)
